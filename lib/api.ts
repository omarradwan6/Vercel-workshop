...
const API_BASE_URL =
  process.env.API_BASE_URL ?? "https://vercel-swag-store-api.vercel.app/api"
...
export async function getProducts(params: GetProductsParams = {}): Promise<Product[]> {
  ...
  if (params.featured !== undefined) qs.set("featured", params.featured ? "true" : "false")
...
export async function getProductById(idOrSlug: string): Promise<Product> {
  return request<Product>(`/products/${encodeURIComponent(idOrSlug)}`, {
    next: { revalidate: DEFAULT_REVALIDATE, tags: ["products", `product:${idOrSlug}`] },
  })
}
...
export async function cartCreate(): Promise<{ token: string; cart: CartWithProducts }> {
  ...
  if (!res.ok) {
    throw new ApiRequestError(res.status, undefined, `cartCreate failed: ${res.status}`)
  }
...
export async function cartGet(token: string): Promise<CartWithProducts> {
  return request<CartWithProducts>("/cart", { cartToken: token, cache: "no-store" })
}
